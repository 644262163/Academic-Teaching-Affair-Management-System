...
<table class="layui-table" lay-data="{height:471, limit: 10, url:'<%=request.getContextPath() %>/teacher/select_course_list.do', page:true, id:'idTest'}" lay-filter="demo">
  <thead>
    <tr>
    ...
    if(obj.event === 'detail'){
      ...
      if(false){
        ...
        layer.confirm('真的删除行么', function(index){
          $.ajax({
            url:'<%=request.getContextPath() %>/teacher/delete_course.do',
            type:'GET', 
            async:false,    //是否异步
            data:{
                id:data.id
      ...
      layer.open({
        ...
        area: ['840px', '420px'], //宽高
        content: '\
          <form class="layui-form layui-form-pane" action="<%=request.getContextPath() %>/teacher/update_course.do">\
  ...
  var $ = layui.$, active = {
    insertCourse: function(){ //添加课程
      layer.open({
        ...
        area: ['840px', '420px'], //宽高
        content: '\
          <form class="layui-form layui-form-pane" action="<%=request.getContextPath() %>/teacher/insert_course.do">\
            <div class="layui-form-item">\
              <label class="layui-form-label">id</label>\
              <div class="layui-input-block">\
