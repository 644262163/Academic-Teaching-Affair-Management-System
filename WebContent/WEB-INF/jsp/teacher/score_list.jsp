...
<table class="layui-table" lay-data="{height:471, limit: 10, url:'<%=request.getContextPath() %>/teacher/select_score_list.do', page:true, id:'idTest'}" lay-filter="demo">
  <thead>
    <tr>
    ...
    if(obj.event === 'detail'){
      ...
      if(false){
        ...
        layer.confirm('真的删除行么', function(index){
          $.ajax({
            url:'<%=request.getContextPath() %>/teacher/delete_score.do',
            type:'GET', 
            async:false,    //是否异步
            data:{
      ...
      layer.open({
        ...
        area: ['840px', '420px'], //宽高
        content: '\
          <form class="layui-form layui-form-pane" action="<%=request.getContextPath() %>/teacher/update_score.do">\
  ...
  var $ = layui.$, active = {
    insertScore: function(){ //添加分数
      layer.open({
        ...
        area: ['840px', '420px'], //宽高
        content: '\
          <form class="layui-form layui-form-pane" action="<%=request.getContextPath() %>/teacher/insert_score.do">\
          <div class="layui-form-item">\
            <label class="layui-form-label">学生id</label>\
              <div class="layui-input-block">\
